var TimeKnots = {
  draw: function(id, events, options){
    ...
    .data(events).enter()
    .append("circle")
    .filter(function(d) { return d.enddate == null })
    ...
    .attr("rx", function(d){
    	if(cfg.horizontalLayout){
    		var datum = (cfg.dateDimension)?new Date(d.date).getTime():d.value;
        	var enddate = (cfg.dateDimension)?new Date(d.enddate).getTime():d.value;
        	return Math.floor(step*(enddate-datum)/2.0)
    	}
    ...
    .attr("cx", function(d){
      ...
      d3.select(this)
      .style("fill", function(d){if(d.color != undefined){return d.color} return cfg.color}).transition()
      .duration(100).attr("ry",  function(d){if(d.radius != undefined){return Math.floor(d.radius*1.5)} return Math.floor(cfg.radius*1.5)});
      tip.html("");
      if(d.img != undefined){
        tip.append("img").style("float", "left").style("margin-right", "4px").attr("src", d.img).attr("width", "64px");
      }
    ...
    .on("mouseout", function(){
        d3.select(this)
        .style("fill", function(d){if(d.background != undefined){return d.background} return cfg.background}).transition()
        .duration(100).attr("ry", function(d){if(d.radius != undefined){return d.radius} return cfg.radius});
        tip.transition()
        .duration(100)
    .style("opacity", 0)});
    ...
    return tip.style("top", (d3.event.pageY-tipPixels-530)+"px").style("left",(d3.event.pageX-20)+"px");})
